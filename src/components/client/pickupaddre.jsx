...
export default class PickupaddRetail extends Component {
    componentDidMount(){
    ...
    clicked(e){
        e.preventDefault(); 
        if(this.state.storename && this.state.additional &&  this.state.type  !==""){
        sessionStorage.setItem('pickstorename', this.state.storename);
        sessionStorage.setItem('pickadd', this.state.additional);
        sessionStorage.setItem('picktype',this.state.type);
        
        window.location.href = "/dropaddr";
        }
        else{
            alert("not add");
        }
            ...
            <div className="px-3 pt-5">
                <div className=" rounded-md bg-white py-5 px-5">
                    ...
                    <div className="border mt-2 rounded-2xl flex">
                        ...
                        <button onClick={() =>window.location.href = "/pickupaddre"} className="basis-1/3 bg-black rounded-2xl text-white py-1" >
                            Retail
                        </button>
                    </div>
                    <form className="mt-4">
                        <div className="relative z-0 mb-6 w-full group">
                            <input type="text" onChange={ (e) => this.setState({ storename: e.target.value})}  className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-600 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="Store Name*" required />
                        </div>
                        <div className="relative z-0 mb-6 w-full group">
